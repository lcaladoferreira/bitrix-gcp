...
class BigQueryClient:
    ...
    def get_watermark(self, table_id: str, field: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Watermark fetch failed: {str(e)}")
    ...
    def create_table_if_not_exists(self, table_id: str, schema_obj: EntitySchema) -> None:
        def _to_bq_field(f):
            if f.get("fields"):
                 return bigquery.SchemaField(f["name"], f["type"], mode=f.get("mode", "NULLABLE"), fields=[_to_bq_field(sub) for sub in f["fields"]])
